...
function XorGateComponent({
  ...
  const output = useMemo(() => xorLogic(inputA, inputB), [inputA, inputB])

  // Gate body color based on state - cyan tinted for XOR
  const bodyColor = useMemo(
    () => (selected ? XOR_BODY_SELECTED : hovered ? XOR_BODY_HOVER : XOR_BODY_COLOR),
    [selected, hovered]
  )
...
export const XorGate = memo(XorGateComponent)
