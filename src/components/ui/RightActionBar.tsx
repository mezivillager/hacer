...
export function RightActionBar({
  ...
  const simulationRunning = useCircuitStore((state) => state.simulationRunning)
      ...
      >
        <div className="flex flex-col h-full" style={{ width: panelWidth }}>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {activePanel === 'info' && <CircuitInfoPanel circuitInfo={circuitInfo} />}
    ...
    <div className="space-y-2">
      {layers.map((layer) => (
        ...
        >
          ...
          <span className="text-xs text-muted-foreground tabular-nums">{layer.count}</span>
        </div>
      ))}
    </div>
  )
