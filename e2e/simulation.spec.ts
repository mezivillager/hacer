...
test.describe('Simulation', () => {
  ...
  test('can start and stop simulation', async ({ page }) => {
    ...
    // Verify store state
    const isRunning = await page.evaluate(() => window.__CIRCUIT_STORE__.simulationRunning)
    ...
    // Verify store state
    const isPaused = await page.evaluate(() => !window.__CIRCUIT_STORE__.simulationRunning)
