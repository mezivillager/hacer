...
- **Invalidates:** when a BLOCK's finding means a past decision (`ADR-NNNN`, `R<n>` or `P-<n>`) was
  ...
  This is what starts the correction workflow (end to end in `docs/decisions/README.md` § Lineage):
  the coordinator runs `lineage radius <id>` on the name to find every decision resting on it.
