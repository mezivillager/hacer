...
| [#199](https://github.com/mezivillager/hacer/issues/199) | Fuzzing is pure compute and embarrassingly parallel — grammar-based and mutated HDL inputs, run at volume. Self-contained. | The parser returns diagnostics and never throws on any generated input; spans stay in range; **the generator is shown failing against a deliberately broken parser** before a green run is trusted. | released | claude-local | done | bc-9878dded-2e89-59b3-9a71-83e34d10257b | Grok Bot please claim as `grok-bot`. **Read the 2026-09-23 comment**: `fast-check` is *not* a dependency and must not be added (that is #198's call) — hand-roll the mutation with a seeded PRNG, as `compiler.test.ts` does. Verify: **local Claude** (`src/core/hdl`, engine). Builder claim released. [PR #444](https://github.com/mezivillager/hacer/pull/444) open (`test/199-hdl-parser-fuzz`). Verify handoff to local Claude (`src/core/hdl`). Grok Bot will not merge. Verified by local Claude (Opus): test sound; the red commit broke production `parseHDL`, so the branch was squashed to one `test(hdl):` commit (identical tree) and **merged as [PR #444](https://github.com/mezivillager/hacer/pull/444)** on 2026-09-24. Follow-ups #453, #454. |
...
| [#468](https://github.com/mezivillager/hacer/issues/468) | Self-contained scripts + a scheduled workflow, and it wants running against the live registry and GitHub API repeatedly while the Verify matching is tuned — cheap in the cloud, a slot on the laptop. Design: `docs/research/2026-09-24-decision-lineage/REPORT.md` §6–§7; the parser it builds on (#487) is merged. | The issue's named tests green; **`node scripts/lineage.mjs verify` on the live register reports P-001 expired** with #342, #408 and PR#409 downstream — the day-one real test; `lineage-verify.yml` on a weekly schedule, never on `pull_request`; PR merged. | claimed-by-grok-bot | claude-local | pr | bc-3b3822f2-583b-53b2-939c-b61bce76c8fc | Grok Bot please claim as `grok-bot`. Verify: **grok-bot** (scripts + workflow, no `src/core`) — the verdict comment must carry the `## Verifier verdict:` heading and a `Verified on:` line (the cross-lane contract, `cloud-queue.md` step 6). Intent: handing-off to local Claude. Claim still `claimed-by-grok-bot` pending Claude's re-claim. Draft [PR #519](https://github.com/mezivillager/hacer/pull/519) head 3753bd1, agent bc-3b3822f2-583b-53b2-939c-b61bce76c8fc; first verify BLOCK (P-005 false-expired) fixed, needs re-verify; coordinator must accept backfilled R398–R405 fields (R405→R400 doubtful); expect conflicts with #514; details in the handing-off comment on #468. |
| [#455](https://github.com/mezivillager/hacer/issues/455) | A CI check that must be *shown failing* on #410's exact React 19.3 / R3F 9.7 pair before it is trusted — an install/verify loop that is slow on the laptop and free in the cloud. | The peer-range check fails on the #410 pair and passes on `main`; allow-listed existing violations carry a reason; the issue's named tests green; PR merged. | released | claude-local | done | bc-ebaee483-e620-50da-be1f-15f9491a96b6 | Grok Bot please claim as `grok-bot`. Verify: **grok-bot**. Second occurrence of the #342 class (ledger 2026-09-23 row); the design options are in the issue. Claim released. [PR #518](https://github.com/mezivillager/hacer/pull/518) merged 2026-09-25 (rebase, main f1418ac), agent bc-ebaee483-e620-50da-be1f-15f9491a96b6, verifier PASS https://github.com/mezivillager/hacer/pull/518#issuecomment-5834287156 . |

**Spending before-shot 2026-09-24 ~03:54 EAT:** Cursor Models 5% / Other Models 94% / Grok Bot weekly 5% (Pro+ reset Oct 19).

**Spending before-shot 2026-09-25 ~11:17 EAT:** Cursor Models 10% / Other Models 99% / Grok Bot weekly 10% (on-demand disabled). **After-shot:** pending.
