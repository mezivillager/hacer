...
| P-007 | over the last 60 merged PRs, 27 carry no `## Verifier verdict:` comment | `gh pr list --state merged --limit 60 --json comments --jq '[.[]|select(([.comments[].body]|map(select(test("Verifier verdict")))|length)==0)]|length'` | `27` | 2026-09-25 (R705) | holds |
