...
// NAND gate specific color (purple/magenta tint)
const NAND_BODY_COLOR = '#5a3d6a'
const NAND_BODY_HOVER = '#7a4d8a'
const NAND_BODY_SELECTED = '#4a9eff'
...
const BUBBLE_RADIUS = 0.12

// Pin positions - aligned to body/bubble boundary
const BODY_LEFT = -BODY_WIDTH / 2   // -0.5
const BODY_RIGHT = BODY_WIDTH / 2   // 0.5
const BUBBLE_CENTER_X = BODY_RIGHT + BUBBLE_RADIUS  // 0.62 (bubble just touching body)
const BUBBLE_RIGHT = BUBBLE_CENTER_X + BUBBLE_RADIUS // 0.74
const INPUT_PIN_X = BODY_LEFT - PIN_RADIUS  // -0.6 (just touching left edge)
const OUTPUT_PIN_X = BUBBLE_RIGHT + PIN_RADIUS // 0.84 (just touching bubble)
...
function NandGateComponent({
  ...
  // Gate body color based on state - purple tinted for NAND
  const bodyColor = useMemo(
    () => (selected ? NAND_BODY_SELECTED : hovered ? NAND_BODY_HOVER : NAND_BODY_COLOR),
    [selected, hovered]
  )
  ...
  return (
    <group ref={groupRef} position={position} rotation={rotation}>
      ...
      {/* Negation bubble - same color as body, just touching the body */}
      ...
      {/* HTML label overlay */}
      <BaseGateLabel gateType="NAND" inputs={[inputA, inputB]} output={output} visible={hovered || selected} />
    </group>
  )
}
