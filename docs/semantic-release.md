...
The release workflow (`.github/workflows/release.yml`) runs automatically when changes are pushed to the `main` branch:
...
4. **Update Version**: Bumps version in `package.json` and `package-lock.json`
...
7. **Commit Changes**: Commits the updated files with `[skip ci]` to avoid triggering another build
